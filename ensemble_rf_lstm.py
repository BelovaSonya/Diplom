# ensemble_rf_lstm.py
"""
Комбинированная модель прогнозирования: Random Forest + LSTM
Использует исторические данные о загруженности парковки для прогнозирования будущих значений
"""
...
class EnsembleRFLSTM:
    ...
    def create_sequences(self, data):
        ...
        Args:
            data: массив временного ряда
        
        Returns:
            X: входные последовательности (samples, lookback, features)
    ...
    def create_features_for_rf(self, time_series, timestamps=None):
        ...
        for i in range(len(time_series) - self.forecast_horizon):
            ...
            window = time_series[max(0, i - self.lookback + 1):i + 1]
            
            # Если окно меньше lookback, дополняем нулями
            if len(window) < self.lookback:
                window = np.pad(window, (self.lookback - len(window), 0), 'constant')
    ...
    def build_lstm_model(self, input_shape):
        ...
        Args:
            input_shape: форма входных данных (lookback, features)
        """
        model = Sequential([
    ...
    def train_lstm(self, time_series):
        ...
        # Изменяем форму для LSTM: (samples, lookback, 1)
    ...
    def train(self, time_series, timestamps=None):
        ...
        # Нормализуем для LSTM
        time_series_normalized = self.scaler_y.fit_transform(time_series.reshape(-1, 1)).flatten()
        
        # Обучаем Random Forest
        rf_success = self.train_rf(time_series, timestamps)
        
        # Обучаем LSTM
        lstm_success = self.train_lstm(time_series_normalized)
    ...
    def predict_rf(self, recent_values, timestamps=None):
        ...
        Returns:
            прогноз на forecast_horizon шагов вперед (первый шаг)
        """
    ...
    def predict_lstm(self, recent_values):
        ...
        Returns:
            прогноз на forecast_horizon шагов вперед
        """
        if self.lstm_model is None:
            return None
        
        # Подготовка входных данных
        window = recent_values[-self.lookback:]
        
        if len(window) < self.lookback:
            window = [window[0]] * (self.lookback - len(window)) + window
        
        # Нормализуем
        window_scaled = self.scaler_y.transform(np.array(window).reshape(-1, 1)).flatten()
    ...
    def predict(self, recent_values, timestamps=None, return_all=False):
        ...
        for i in range(self.forecast_horizon):
            if rf_pred is not None and i == 0:
...
class EnsemblePredictorVisualizer:
    ...
    def plot_predictions(self, actual_values, predictions, title="Прогноз загрузки парковки"):
        ...
        if hasattr(self.ensemble, 'lstm_model') and self.ensemble.lstm_model:
        ...
        # 4. История обучения LSTM (если есть)
        ax4 = axes[1, 1]
        ax4.text(0.5, 0.5, f'LSTM архитектура\n'
                           f'Lookback: {self.ensemble.lookback}\n'
                           f'Forecast: {self.ensemble.forecast_horizon}',
                ha='center', va='center', fontsize=12,
                bbox=dict(boxstyle='round', facecolor='lightyellow'))
        ax4.set_title('Параметры LSTM', fontsize=12)
...
def main():
    ...
    if success:
        ...
        if predictions:
            ...
            print("-" * 50)
            print(f"{'Шаг':<6} {'RF':<10} {'LSTM':<10} {'Ансамбль':<12}")
            print("-" * 50)
            ...
            for i in range(FORECAST_HORIZON):
                ...
                print(f"{i+1:<6} {rf_str:<10} {lstm_val:<10} {ensemble_val:<12}")
